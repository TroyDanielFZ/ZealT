...
{
    qDebug() << ("DocsetRegistry::loadDocset " + path);
    ...
    connect(watcher, &QFutureWatcher<Docset *>::finished, this, [this, watcher] {
        ...
        // TODO: Emit error
        if (!docset->isValid()) {
            qWarning("Could not load docset from '%s'. Reinstall the docset.", qPrintable(docset->path()));
            qDebug() << QString("Could not load docset from '%1'. Reinstall the docset.").arg(qPrintable(docset->path()));
            delete docset;
            return;
        }
