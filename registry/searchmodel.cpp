...
{
    for(int i = 0; i< 36; ++i){
        m_indexIcon.push_back(new QIcon(i<10? QStringLiteral(":/icons/index/%1.svg").arg((i+1)%10 ):
                                          QStringLiteral(":/icons/index/%1.svg").arg(QChar(i-10 + 65))));
    }
    m_indexIcon.push_back(new QIcon(QStringLiteral(":/icons/index/empty.svg")));
}
...
QVariant SearchModel::data(const QModelIndex &index, int role) const
{
    ...
    case ItemDataRole::DocsetIconRole:
        return  index.row() > 36 ? item->docset->icon(): (*m_indexIcon[index.row()]); // if having index icon, return it, otherwise the role icon
