    ...
    m_path(path)
{
    ...
    qDebug() << "Starting to parse Docset: " << path;
    loadMetadata();
    ...
    if (!dir.cd(QStringLiteral("Contents"))){
        qDebug() << "Unable to find dir \"Contents\".";
        return;
    }
    ...
    if (dir.exists(QStringLiteral("Info.plist")))
        ...
        qDebug() << "Unable to find file \"info.plist\".";
        return;
    }

    if (plist.hasError()){
        qDebug() << "Error parsing file \"info.plist\".";
        return;
    }
    ...
    if (!dir.cd(QStringLiteral("Resources")) || !dir.exists(QStringLiteral("docSet.dsidx"))){
        qDebug() << "No dir \"Resources\", or no file \"docSet.dsidx\".";
        return;
    }
    ...
    if (!m_db->isOpen()) {
        ...
        qDebug() << "Open \"docSet.dsidx\" as sqlite failed.";
        return;
    }
    ...
    if (!dir.cd(QStringLiteral("Documents"))) {
        m_type = Type::Invalid;
        qDebug() << "Error enter dir \"Documents\".";
        return;
    }
    ...
    if (plist.contains(InfoPlist::DashIndexFilePath)) {
        ...
        if (dir.exists(QStringLiteral("index.html")))
            ...
            qDebug() << QString("Cannot determine index file for docset %1.").arg(qPrintable(m_name));
...
QList<SearchResult> Docset::search(const QString &query, const CancellationToken &token) const
{
    ...
    // todo: add support to multiple queries entries
    // ref:https://stackoverflow.com/questions/43944656/search-sqlite3-database-using-multiple-words
    QStringList queries = sanitizedQuery.split(' ');
//    queries.sort();
    QString sql;
    if(!query.length() || !queries.size()){
//        return QList<SearchResult>();
        sql = QStringLiteral("SELECT name, type, path, '', -length(name) as score"
                             "  FROM searchIndex  LIMIT 1000;");
    }else{// non-empty queries
        ...
        sql += QString("name LIKE '%%1%' ESCAPE '\\' ").arg(queries[0]);
        for(auto it = queries.begin() + 1; it != queries.end(); ++it){
            auto qr = it->trimmed();
            if(qr.size()>0)
                sql += QString(" AND name LIKE '%%1%' ESCAPE '\\'").arg(qr);
        }
