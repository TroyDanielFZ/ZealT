...
const char GroupAssociation[] = "associations";
}
...
void Settings::load()
{
    ...
    // GroupAssociation
    QDir zealDir(qApp->applicationDirPath());
    settings->beginGroup(GroupAssociation);
    QStringList filetypes = settings->value("filetypes", "").toStringList();
//mapFileAssociations
    foreach(auto type, filetypes){
        AssociationInfo a = AssociationInfo(type,
			settings->value(type + "_command", "").toString(),
            settings->value(type + "_spliter", "|").toString(),
            settings->value(type + "_regex", "").toString(),
            settings->value(type + "_match", "").toString());
        lstFileAssociations.push_back(a);
        qDebug() << "Settings filetype: "
			<< "\n\ttype - " << a.type
			<< "\n\tcommand - " << a.command
			<< "\n\tspliter - " << a.spliter
			<< "\n\tregex - " << a.regex
...
//        QString exePath = settings->value(key).toString();;
//        mapFileAssociations.insert(key, exePath);
//    }
    settings->endGroup();
