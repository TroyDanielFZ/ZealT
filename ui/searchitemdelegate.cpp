                               ...
                               const QModelIndex &index) const
{
    ...
    if (!m_highlight.isEmpty()) {
        ...
        // TDtodo: highlight several queries
        QStringList queries = m_highlight.split(' ');
        foreach(auto query, queries){
            query = query.trimmed();
            for (int i = 0;;) {
                const int matchIndex = opt.text.indexOf(query, i, Qt::CaseInsensitive);
                if (matchIndex == -1 || matchIndex >= elidedText.length() - 1)
                    break;

                QRect highlightRect
                        = textRect.adjusted(fm.width(elidedText.left(matchIndex)), 2, 0, -2);
                highlightRect.setWidth(fm.width(elidedText.mid(matchIndex, query.length())));
